...
//		 System.out.println (new BinaryGap().solution(2567));
		System.out.println(new BinaryGap().solution(890231));
	...
	public int solution(int N) {
		...
		for (int i = 1; i < ch.length - 1; i++) {
			...
			if (i < ch.length - 1) {
				if (ch[i] == '0' && ch[i + 1] == '1') {
					endIndex = i + 1;
					char[] temp = binary.substring(startIndex, endIndex + 1).toCharArray();
